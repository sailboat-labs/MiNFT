    ...
    <div>
      <div className="mb-8 h-[55vh]">
        ...
        <div className="flex items-center justify-center text-white">
          <div className="relative flex h-80 w-3/5 overflow-hidden mdlg:w-3/5">
            <button
              className="absolute inset-y-1/2 left-0 cursor-default text-3xl text-white"
              onClick={previous}
            >
              <PreviousSVG className="cursor-pointer text-white" />
            </button>

            {slides.map((slide, index) => (
              <Hammer key={index} onSwipeLeft={next} onSwipeRight={previous}>
                <div
                  id="slider"
                  key={index}
                  className={
