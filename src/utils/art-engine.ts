...
export function generateTokens({
  ...
  return new Promise(async (resolve) => {
    ...
    const filterDNAOptions = (_dna: string) => {
      const filteredDNA = _dna
        ...
        .filter((element: string) => {
          ...
          const options: any = querystring[1]
            .split("&")
            .reduce((r, setting) => {
    ...
    const sortLayers = (layers: any[]) => {
      ...
      return sortByZ(stack.front)
        .concat(stack.normal)
        .concat(sortByZ(stack.end));
    };
    ...
    const writeMetaData = (_data: string) => {
      debugLogs && console.log("write metadata");
    ...
    const writeDnaLog = (_data: string) => {
      ...
      debugLogs && console.log("write dna");
    };
    ...
    const saveMetaDataSingleFile = (_editionCount: any) => {
      ...
      debugLogs
        ...
        : null;
      debugLogs && console.log("write single metadata");
    ...
    const outputFiles = (
      ...
      generatedTokens.push(generatedToken);
    ...
    function generateDeterministicDNA() {
      const res = [];
    ...
    const startCreating = async () => {
      return generateDeterministicDNA();
      let usingDNACount = 0;
      generatedTokens = [];
      try {
